...
describe('Pawn', () => {
    ...
    test.each([
        ["Black", 0, 6, 1],
        ["White", 1, 7, -1]
    ])("%s Pawn is Moveable", (color: string, min: number, max: number, added: number) => {
    ...
    test.each([
        ["Black", 0, 6, 1],
        ["White", 1, 7, -1]
    ])("%s Pawn moveTo valid", (color: string, min: number, max: number, added: number) => {
        for (let i: number = 0; i < 10; i++) {
            const x = randomIntFromInterval(0, 7);
            const y = randomIntFromInterval(min, max);
            const pawn: Pawn = new Pawn(new Position(x, y), color == 'White' ? 'white' : 'black' );
    ...
    test.each([
        ["Black", 0, 6, 1],
        ["White", 1, 7, -1]
    ])("%s Pawn moveTo invalid", (color: string, min: number, max: number, added: number) => {
        for (let i: number = 0; i < 10; i++) {
            const x = randomIntFromInterval(0, 7);
            const y = randomIntFromInterval(min, max);
            const currentPosition = new Position(x, y)
            const pawn: Pawn = new Pawn(currentPosition, color == 'White' ? 'white' : 'black' );
